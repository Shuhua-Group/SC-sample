...
int Edge::getMutNum(){
    NodePtr pTopNode = getTopNodeRef();
    NodePtr pBottomNode =  getBottomNodeRef();
    GeneString* pTopGS = (GeneString*)pTopNode->getAssoData();
    GeneString* pBottomGS = (GeneString*)pBottomNode->getAssoData();
    if(pTopGS != NULL && pBottomGS != NULL)
    {
     return pBottomGS->diff(*pTopGS);          
    }else{
      //cerr<<"Exception: xgetMutNum pTopGS or pBottomGS is NULL!"<<endl;
      //wy added
      if(pTopGS == NULL && pBottomGS == NULL)
      {cerr<<"Exception: xgetMutNum pTopGS and pBottomGS are both NULL!"<<endl;}
      //wy added end
      return 0;      
    }            
...
GraphBuilder::GraphBuilder(Configuration *pConfig,RandNumGenerator * pRG, SamplesBySite* pSample){
    //llf add
    this->pSample = pSample;
    this->pMutSolver = new StdMutSolver(this);
    nValueCnt[0] = 0;
    nValueCnt[1] = 0;
    double dCurPos = 0.0;
    iCurrentTreeId = 0;
...
EdgePtr & selectedEdge){
    ...
    //llf added
    //set genestring of bottom node as new xOver node, right???
    if(bottomNodeCopy == NULL)
      throw "bottomNodeCopy is null";
    GeneString *pBtmNodeGS = (GeneString*)bottomNodeCopy->getAssoData();
    if(pBtmNodeGS == NULL)
      throw "GeneString of bottomNodeCopy is null";
    //else
    //  cerr<<"GeneString of bottomNodeCopy is:"<<pBtmNodeGS->c_str()<<endl;
    GeneString *pNewNodeGS = new GeneString(*((GeneString*)bottomNodeCopy->getAssoData()));
    newNode->setAssoData(pNewNodeGS);
    //llf added end
