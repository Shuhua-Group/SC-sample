...
//    enum ValueType {V_ZERO, V_ONE, V_UNKNOWN};
...
//    void clearBottomEdges();
    ...
    //llf added
    void setAssoData(void* p){if(pData){delete (GeneString*)pData; pData = NULL;}; pData = p;};
    const void* getAssoData(){return pData;};
    void setCData(char c){cData = c;};
    char getCData(){return cData;};
    //llf added end
private:
    EventPtr event;
...
public:
    ...
    GraphBuilder(Configuration *,RandNumGenerator *, SamplesBySite *);
    ~GraphBuilder();
...
private:
    ...
    //llf added
    SamplesBySite *pSample;
    StdMutSolver *pMutSolver;
    int nValueCnt[2];
    double dCurPos;
    EdgePtr selectRandomEdgeWithMutation(double& dSplitPoint);
//    int nValueOneCnt;
...
//    int iGraphIteration;
    ...
    void invokeRecombination(GeneConversionPtr & ,NodePtr & preXoverNode);
    ...
    NodePtr TracebacktoCurrentTree(EdgePtr Edge);
    EdgePtr getAcceptableEdgeToCoalesce(EdgePtr & coalescingEdge,EdgePtr xOverEdge,double dHeight,NodePtr pRoot, SamplesBySite& samples, double dStartPos);
    void eraseEdgeVector(EdgePtrVector& EdgeVector, NodePtr XoverEdgeTopNode);
    bool isMutEdge(NodePtr XoverEdgeTopNode);
    void getMutEdgeVector(EdgePtrVector& mutEdgeVector);
    double TopAcceptEdgeHeight(NodePtr Node,NodePtr pRoot, SamplesBySite& samples, double dStartPos);
    NodePtr getXoverBtmNode(EdgePtr Edge);
